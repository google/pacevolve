...
def _find_latest(search_dirs: list[str], filename: str) -> str | None:
...
def _summarize_float_checkpoint(path: str | None) -> dict[str, object]:
    ...
    try:
        ...
        tensor_stats = _tensor_summary(state_dict if isinstance(state_dict, dict) else {})
        summary.update(tensor_stats)
...
def _summarize_quantized_artifact(path: str | None) -> dict[str, object]:
    ...
    try:
        ...
        summary["per_row_quant_tensors"] = float(
            sum(1 for name, scale in scales.items() if qmeta.get(name, {}).get("scheme") == "per_row" or getattr(scale, "ndim", 0) > 0)
        )
        tensor_stats = _tensor_summary(quantized)
        summary["quantized_param_count"] = tensor_stats["param_count"]
        summary["quantized_int8_abs_mean"] = tensor_stats["abs_mean"]
        summary["quantized_int8_zero_fraction"] = tensor_stats["zero_fraction"]
...
def build_artifact_info(src_path: str, eval_output: str) -> dict[str, object]:
    search_dirs = []
    for candidate_dir in [src_path, os.getcwd()]:
        candidate_dir = os.path.abspath(candidate_dir)
        if candidate_dir not in search_dirs:
            search_dirs.append(candidate_dir)

    float_ckpt_path = _find_latest(search_dirs, "final_model.pt")
    quant_artifact_path = _find_latest(search_dirs, "final_model.int8.ptz")
...
def analyze_candidate(candidate_source: str, eval_output: str, artifact_info: dict[str, object]) -> dict[str, float]:
    ...
    candidate_bpb = metric(summary, "candidate_val_bpb")
...
def main() -> int:
    ...
    try:
        ...
        artifact_info = build_artifact_info(args.src_path, eval_output)
