...
def parse_eval_results(
  ...
  """
  Parses a string or list of strings to extract the score used for evolution.
  We optimize for lower training time once target validation quality is met.
  """
  if isinstance(eval_results, str):
    ...
    return metrics.get("candidate_validation_loss")
...
def parse_eval_metrics(eval_results: list[str] | str) -> dict[str, float]:
  if isinstance(eval_results, str):
    ...
    fields = {
      "candidate_validation_loss": r"Candidate validation loss:\s*(-?\d+(?:\.\d+)?(?:e[+-]?\d+)?)",
      "training_time": r"Training time:\s*(-?\d+(?:\.\d+)?(?:e[+-]?\d+)?)",
      "baseline_validation_loss": r"Baseline validation loss:\s*(-?\d+(?:\.\d+)?(?:e[+-]?\d+)?)",
      "baseline_training_time": r"Baseline training time:\s*(-?\d+(?:\.\d+)?(?:e[+-]?\d+)?)",
    }
